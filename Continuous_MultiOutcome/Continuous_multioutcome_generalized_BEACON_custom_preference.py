...
def reachability_uniformity(xl, xu, behavior):
    # Example tensors for lower and upper bounds (NxD)
    xL = torch.tensor([[0, 0], [2, 2], [4, 4]], dtype=torch.float32)  # Lower bounds of hypercubes
    xU = torch.tensor([[1, 1], [3, 3], [5, 5]], dtype=torch.float32)  # Upper bounds of hypercubes
    
    # Example tensor for elements (nxD)
    behavior = torch.tensor([[0.5, 0.5], [2.5, 2.5], [4.5, 4.5], [6, 6]], dtype=torch.float32)
    ...
    num_filled_hypercubes = filled.sum().item()
