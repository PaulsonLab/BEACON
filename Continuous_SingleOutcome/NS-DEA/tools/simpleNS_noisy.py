...
class Experiment:
    ...
    def gene_to_behavior(self, g):
        ...
        elif self.mapping == 'Hartmann':
            func = Hartmann(dim=self.dim)
            behavior = func(self.lb+(self.ub-self.lb)*torch.tensor(g)).numpy()
    ...
    def GP(self):
        ...
        self.model = SingleTaskGP(train_x, train_y, outcome_transform=Standardize(m=1), covar_module=covar_module, likelihood=likelihood)
