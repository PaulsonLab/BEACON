...
if __name__ == '__main__':
    ...
    for seed in range(replicate): 
        ...
        for i in range(BO_iter):        
            ...
            model = SingleTaskVariationalGP(train_X=train_x, train_Y = train_y, covar_module=covar_module, outcome_transform=Standardize(m=1), inducing_points=int(0.25*train_x.shape[-2]), learn_inducing_points=False)
    ...
    torch.save(coverage_tensor, '4DAckley_coverage_list_NS_TS5.pt')
    torch.save(cost_tensor, '4DAckley_cost_list_NS_TS5.pt')  
    torch.save(time_tensor, '4DAckley_time_list_NS_TS5.pt')
