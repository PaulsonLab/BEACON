...
class Experiment:
    ...
    def __init__(self, params, display=False, seed = None, dim = 2, obj_lb = -5, obj_ub = 5, lb = -5, ub = 5):
        ...
        self.archive = np.full((self.n_epochs + 1, self.n_selected, self.I_BEHAVIOR_true+1), np.nan, dtype=np.float32)
        self.population = np.full((self.n_epochs + 1, self.n_pop, self.I_BEHAVIOR_true+1), np.nan, dtype=np.float32)
        self.offsprings = np.full((self.n_epochs, self.n_offspring, self.I_BEHAVIOR_true+1), np.nan, dtype=np.float32)
    ...
    def gene_to_behavior(self, g):
        ...
        if self.mapping=='Rosen':
            func = Rosenbrock(dim=self.dim, noise_std=self.noise)
            behavior = func(self.lb+(self.ub-self.lb)*torch.tensor(g)).numpy()
            
        elif self.mapping == "Ackley":
            func = Ackley(dim = self.dim, noise_std=self.noise)
            behavior = func(self.lb+(self.ub-self.lb)*torch.tensor(g)).numpy()
    
        return behavior
    
    def gene_to_behavior_true(self, g):
        """Non-linear mapping from genome to behavior in [-5, 5]
    ...
    def GP(self, train_x, train_y):
      
        train_x = torch.tensor(train_x)
        train_y = torch.tensor(train_y).unsqueeze(1)
        covar_module = ScaleKernel(RBFKernel(ard_num_dims=self.dim)) # select the RBF kernel
        likelihood = gpytorch.likelihoods.GaussianLikelihood()
        self.model = SingleTaskGP(train_x.to(torch.float64), train_y.to(torch.float64), outcome_transform=Standardize(m=1), covar_module=covar_module, likelihood=likelihood)
    ...
    def initialize_population(self):
        ...
        self.population[self.t, :, self.I_BEHAVIOR_true] = self.gene_to_behavior_true(self.population[self.t, :, :][:, self.I_GENOME])
        self.population[self.t, :, self.I_DISTANCE] = np.zeros(self.n_pop, dtype=np.float32)
        self.GP(self.population[self.t, :, :][:, self.I_GENOME], self.population[self.t, :, self.I_BEHAVIOR])
        new_b = self.model.posterior(torch.tensor(self.population[self.t, :, :][:, self.I_GENOME])).mean
        old_b = new_b
        self.population[self.t, :, self.I_NOVELTY] = self.compute_novelty(new_b.flatten().detach().numpy(),
                                                                     old_b=old_b.flatten().detach().numpy())  # compare only to itself
    ...
    def generate_offsprings(self):
        ...
        self.offsprings[self.t, :, self.I_BEHAVIOR_true] = self.gene_to_behavior_true(self.offsprings[self.t, :, :][:, self.I_GENOME])
        ...
        train_x = extended_population[:, self.I_GENOME]
        train_y = extended_population[:, self.I_BEHAVIOR]
        self.GP(train_x, train_y)
        old_GENOMES = torch.tensor(self.get_reference_behaviors())
        old_behaviors = self.model.posterior(old_GENOMES).mean
        new_behaviors = self.model.posterior(torch.tensor(self.offsprings[self.t, :, :][:, self.I_GENOME])).mean
        
        self.offsprings[self.t, :, self.I_NOVELTY] = self.compute_novelty(new_behaviors.flatten().detach().numpy(), old_behaviors.flatten().detach().numpy())
    ...
    def get_reference_behaviors(self):
        ...
        """
        return np.vstack((self.population[self.t, :, :][:, self.I_GENOME],
                          self.offsprings[self.t, :, :][:, self.I_GENOME],
                          self.archive[self.t, :, :][:, self.I_GENOME]))
    ...
    def evaluate_coverage_and_uniformity_custom(self, g):
        ...
        """
        num = g.shape[0]
        all_behavior_samples = g[:, self.I_BEHAVIOR_true]
    ...
    def run_novelty_search(self):
        ...
        while self.t < self.n_epochs:
            ...
            old_GENOMES = torch.tensor(self.get_reference_behaviors())
            old_behaviors = self.model.posterior(old_GENOMES).mean
            new_behaviors = self.model.posterior(torch.tensor(self.population[self.t, :, :][:, self.I_GENOME])).mean
            self.population[self.t, :, self.I_NOVELTY] = self.compute_novelty(new_behaviors.flatten().detach().numpy(), old_behaviors.flatten().detach().numpy())
