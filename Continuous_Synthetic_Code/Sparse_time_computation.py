...
if __name__ == '__main__':
    ...
    N_init = 2500 # number of initial training data
    replicate = 10 # number of replicates for experiment
    BO_iter = 1 # number of evaluations
    ...
    for seed in range(replicate): 
        ...
        for i in range(BO_iter):        
            ...
            #     training_data_scale, search_space, num_inducing_points=800
            # )
            ...
            end_time = time.time()
            # print(end_time_TS - start_time_TS) 
                
            trajectory_sampler = model.trajectory_sampler()
            TS_func = trajectory_sampler.get_trajectory()
            
            TS_func(search_space.sample(1))
            ...
            for _ in range(multi_start):
                ...
                optimizer = tf.keras.optimizers.Adam(0.1)
                for step in tqdm(range(3), disable=True):
                    with tf.GradientTape(watch_accessed_variables=False) as tape:
                        tape.watch(Xvars)
    ...
    torch.save(time_tensor, '20DRosen_time_list_NS_GPflow_time_2500.pt')    
