...
class CustomAcquisitionFunction(AcquisitionFunction):
    ...
    def hesgau(self, x, y, sigma):
        dim = len(x)
        # dist = np.sum(np.power(x-y, 2))
        dist = torch.sum(torch.pow(x-y,2))
        # return (dim/sigma - dist/sigma**2)*np.exp(-dist/(2*sigma))
        return (dim/sigma - dist/sigma**2)*torch.exp(-dist/(2*sigma))



    def stein_novelty(self, point, data_list, sigma):
        n = len(data_list)
        score = 0
        # score = np.sum([self.hesgau(point, data_list[k,:], sigma) for k in range(n)])
        score = torch.sum(torch.stack([self.hesgau(point, data_list[k,:], sigma) for k in range(n)]))
        # dist = torch.pow(torch.cdist(point, data_list), 2)
        # score = torch.sum((dim/sigma - dist/sigma**2)*torch.exp(-dist/(2*sigma)))
    ...
    def forward(self, X):
        ...
        # acquisition_values = torch.stack([self.stein_novelty(point, self.sampled_behavior, sigma=1) for point in samples])
...
if __name__ == '__main__':
    ...
    # Case study 1
    # feat = set(["func-chi-0-all" ,"D_func-S-3-all", "total_SA_volumetric", 
    ...
    torch.save(cost_tensor, 'MOF_Stein_cost_list.pt')  
    torch.save(coverage_tensor, 'MOF_Stein_coverage_list.pt')  
