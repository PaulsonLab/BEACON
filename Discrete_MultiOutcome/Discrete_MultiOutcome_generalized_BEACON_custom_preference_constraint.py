    ...
    for seed in range(replicate):
        ...
        for i in range(BO_iter):        
            ...
            if coverage<1:
                ...
                samples_val = custom_acq_function.samples
                samples_val[:,0][samples_val[:,0]<mins[0]] = mins[0]
                samples_val[:,1][samples_val[:,1]<mins[1]] = mins[1]
                samples_val[:,0][samples_val[:,0]>maxs[0]] = maxs[0]
                samples_val[:,1][samples_val[:,1]>maxs[1]] = maxs[1]
                ids_sorted_by_aquisition = acquisition_values.argsort(descending=True)
                loop=0
                for id_max_aquisition_all in ids_sorted_by_aquisition: 
                    if not id_max_aquisition_all.item() in ids_acquired: # make sure we don't re-sample point
                        _, fill_TS = reachability_uniformity(xl, xu, samples_val[id_max_aquisition_all].unsqueeze(0))
                        already_filled = torch.any(fill_TS & filled_total).item()
                        if already_filled:
                            loop+=1
                            # print('TS sample already filled!')
                        else:
