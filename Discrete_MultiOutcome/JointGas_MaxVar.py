...
if __name__ == '__main__':    
    ...
    lb = torch.tensor(data1[list(feat)].values.min(axis=0))
    ub = torch.tensor(data1[list(feat)].values.max(axis=0))
    X_original = (torch.tensor(data1[list(feat)].values) - lb)/(ub-lb)
