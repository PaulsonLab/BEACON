...
class CustomAcquisitionFunction(AcquisitionFunction):
    ...
    def forward(self, X):
        ...
        return acquisition_values.flatten(), samples
    ...
    for iter_BO in range(40):
        ...
        with _fast_solves(True):
            fit_gpytorch_mll_torch(
                mtgp_mll, step_limit=1000, optimizer=partial(Adam, lr=0.05)
            )
        
      
        custom_acq_function = CustomAcquisitionFunction(mtgp, mtgp_train_y, k_NN=10)
        acquisition_values, sample = custom_acq_function(X_original)
        ids_sorted_by_aquisition = acquisition_values.argsort(descending=True)
        for id_max_aquisition_all in ids_sorted_by_aquisition:
            if not id_max_aquisition_all.item() in ids_acquired:
                # test_output, last_layer = cnn_trained(sample[id_max_aquisition_all].cpu().unsqueeze(0).unsqueeze(1))
                # pred_y1 = torch.max(test_output, 1).indices[torch.max(test_output, 1).values>=0.5]
                # if torch.isin(torch.tensor(np.unique(pred_y)), pred_y1).any().item():
                #     continue
                # else:
