...
if __name__ == "__main__":
    ...
    for seed in range(replicate):
        ...
        coverage, cost = run_sequentially(parameters,seed,dim,obj_lb,obj_ub,lb,ub)
        cost_list.append(cost)
        coverage_list.append(coverage)
        
        # file_path = save_experiment_results(parameters, data)
    end_time = time.time()
    coverage_list = torch.tensor(coverage_list).to(torch.float64)
    ...
    torch.save(coverage_list, 'MNIST_coverage_list_NSEA.pt')
    # torch.save(uniformity_list, 'MediumMaze_uniformity_list_GA_DEA.pt')
    torch.save(cost_list, 'MNIST_cost_list_NSEA.pt')
