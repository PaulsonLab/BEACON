...
class CustomAcquisitionFunction():
    ...
    def __call__(self, X):
        ...
        samples = posterior.rsample(sample_shape=torch.Size([1])).mean(dim=1) # Need to take the average accross all MCMC samples
        ...
        E = torch.cat((torch.ones(self.k), torch.zeros(n-self.k)), dim = 0) 
        dist = dist*E
