...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 23:53:24 2024
...
class CustomAcquisitionFunction():
    ...
    def __call__(self, X):
        ...
        samples = posterior.mixture_mean.unsqueeze(0)
        # samples = posterior.rsample(sample_shape=torch.Size([1])) # Thompson Sampling (posterior is on original scale) https://botorch.org/api/_modules/botorch/generation/sampling.html https://github.com/pytorch/botorch/blob/main/botorch/models/gp_regression.py
        # samples = posterior.rsample(sample_shape=torch.Size([1])).mean(dim=2) # SaasBO (need to take the mean for all MCMC samples)
...
if __name__ == '__main__':
    ...
    torch.save(coverage_tensor, 'logD_coverage_list_NS_SAAS_mean.pt')
    torch.save(uniformity_tensor, 'logD_uniformity_list_NS_SAASS_mean.pt')
    torch.save(cost_tensor, 'logD_cost_list_NS_SAASS_mean.pt')  
